...
@section('page-title', '評價管理')

@section('page-content')
    <div class="container-fluid px-4">
        <h1 class="mt-4">評價管理</h1>
        <div class="container px-4 px-lg-5 mt-2 mb-4">
            <form action="{{ route('sellers.comments.search') }}" method="GET" class="d-flex">
        ...
        @if (request()->has('query'))
            <div class="container px-4 px-lg-5 mt-2 mb-4">
                查找「{{ request('query') }}」
                <a class="btn btn-success btn-sm" href="{{ route('sellers.comments.index') }}">取消搜尋</a>
            </div>
        @endif
        <table class="table">
            <thead>
            ...
            @foreach($orders as $index => $order)
                <tr>
                    ...
                    <td style="text-align:center">
                        <a href="{{ route('sellers.comments.edit',$order->id) }}" class="btn btn-secondary btn-sm">檢視評價</a>
                    </td>
